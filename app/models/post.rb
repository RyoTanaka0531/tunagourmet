class Post < ApplicationRecord
    ...
    def create_notification_comment!(current_producer, comment_id)
        ...
        temp_ids.each do |temp_id|
            save_notification_comment!(current_producer, comment_id, temp_id['producer_id'])
        end
        # まだ誰もコメントしていない場合は、投稿者に通知を送る
        save_notification_comment!(current_producer, comment_id, producer_id) if temp_ids.blank?
    end

    def save_notification_comment!(current_producer, comment_id, visited_id)
        # コメントは複数回することが考えられるため、１つの投稿に複数回通知する
        notification = current_producer.active_notifications.new(
            post_id: id,
            comment_id: comment_id,
