class Post < ApplicationRecord
    ...
    # def create_notification_by(current_buyer)
    #     temp = Notification.where(["visiter_id = ? and visited_id = ? and post_id = ? and action = ? ", current_buyer.id, buyer_id || producer_id, id, 'like'])
    #     if temp.blank?
    #         notification = current_buyer.active_notifications.new(
    #             post_id: id,
    #             visited_id: buyer_id || producer_id,
    #             action: 'like'
    #         )
    #         if notification.visiter_id == notification.visited_id
    #             notification.checked = true
    ...
    # def save_notification_comment!(current_buyer, comment_id, visited_id)
    ...
    #     notification.save if notification.valid?
    # end
    def save_notification_comment!(current_buyer, comment_id, visited_id, visited_producer_id)
        # コメントは複数回することが考えられるため、１つの投稿に複数回通知する
        notification = current_buyer.active_notifications.new(
            post_id: id,
            comment_id: comment_id,
            visited_id: visited_id,
            visited_producer_id: visited_producer_id,
            action: 'comment'
        )
        # 自分の投稿に対するコメントの場合は、通知済みとする
        if notification.visiter_id == notification.visited_id
    ...
    def create_notification_producer_comment!(current_producer, comment_id)
        ...
        temp_ids.each do |temp_id|
            save_notification_producer_comment!(current_producer, comment_id, temp_id['producer_id' || 'buyer_id'])
        end
        # まだ誰もコメントしていない場合は、投稿者に通知を送る
        save_notification_producer_comment!(current_producer, comment_id, producer_id, buyer_id) if temp_ids.blank?
    end

    def save_notification_producer_comment!(current_producer, comment_id, visited_id, visited_producer_id)
        # コメントは複数回することが考えられるため、１つの投稿に複数回通知する
        notification = current_producer.active_notifications.new(
            post_id: id,
            comment_id: comment_id,
